 ...
 * Plugin URI:        https://irunstuff.com/wordpress-plugins/comment-runner
...
class Irunstuff_Comment_Handler_Plugin {
  function __construct() {
    ...
    add_filter( 'pre_comment_approved' , array( $this, 'irunstuff_comment_handler' ), 100, 2 );

    add_action( 'wp_enqueue_scripts', array( $this, 'comment_css' ) );
  }
  ...
  function irunstuff_comment_handler( $approved, $comment_data ) {
    ...
    $has_phone_number = stristr( $comment_content, 'tel:' );
    $has_a_tag = stristr( $comment_content, 'href=' );

    if ( ( $has_link || $has_mailto || $has_phone_number || $has_a_tag ) && ! current_user_can('administrator') ) {
...
$irunstuff_comment_handler_plugin = new Irunstuff_Comment_Handler_Plugin();
